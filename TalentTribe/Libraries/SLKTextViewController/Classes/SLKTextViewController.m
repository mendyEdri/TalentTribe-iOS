...
- (void)slk_setScrollView:(UIScrollView *)scrollView {
    [self setScrollViewProxy:scrollView];
    
    [self.scrollViewProxy removeFromSuperview];
...
- (void)slk_willShowOrHideKeyboard:(NSNotification *)notification
{
    ...
    self.keyboardHC.constant = [self slk_appropriateKeyboardHeight:notification] - (status == SLKKeyboardStatusWillHide ? self.textInputbarHC.constant : 0.0f);
    self.scrollViewHC.constant = [self slk_appropriateScrollViewHeight];
